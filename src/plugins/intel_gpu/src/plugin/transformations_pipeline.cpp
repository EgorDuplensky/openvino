...
void TransformationsPipeline::apply(std::shared_ptr<ov::Model> func) {
...

    {
        ...
        const bool disable_moe_opt_env = std::getenv("OV_GPU_DISABLE_MOE_OPT") != nullptr;
        ...
        if (device_info.supports_immad) {            // GatherMatmul is only implemented for systolic GPUs, so keep the whole MOE rewrite
            ...
            if (!disable_moe_opt_env && !disable_moe_opt) {
                manager.register_pass<ov::pass::MoeOpFusion>(has_batch_dim);
                const bool disable_moe_fused_opt_env = std::getenv("OV_GPU_DISABLE_MOE_FUSED_OPT") != nullptr;
                if (!disable_moe_fused_opt_env) {
