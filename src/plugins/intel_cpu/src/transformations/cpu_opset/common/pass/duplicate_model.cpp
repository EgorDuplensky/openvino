    ...
    public:
        void add_duplicate(const std::shared_ptr<ov::Node>& node,
            ...
            for (const auto& input : node->inputs()) {
                ...
                if (m_output_clones.count(input_source_output)) {
                ...
                } else if (!op::util::is_on_constant_path(input.get_source_output()) && !ov::is_type<ov::op::internal::Placeholder>(input_node)) {
                    ...
                    std::cout << "Input node: " << v(input_node) << " adding parameter: " << v(m_parameters.back()) << "\n";
                } else {
            ...
            if (clone->get_friendly_name() == "__module.model.layers.0.self_attn.rotary_emb/aten::matmul/MatMul_clone_0") {
                std::cout << clone->get_friendly_name() << "\n";
            }
            ...
            if (auto read_value = ov::as_type_ptr<ov::op::v6::ReadValue>(clone)) {
                ...
                auto new_variable_id =  variable->get_info().variable_id + "_clone_" + std::to_string(numa_id);
                ...
                variable_clones[variable->get_info().variable_id][numa_id] = variable_clone;
            }
        }
        ...
        void build(const std::shared_ptr<ov::Node>& node,
            ...
            std::cout << "Creating subgraph: " << node->get_friendly_name() << "_model" << " Parameters: ";
