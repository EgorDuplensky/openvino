...
DnnlPostOpsComposerNew::DnnlPostOpsComposerNew(const PostOps& postOps,
...
bool DnnlPostOpsComposerNew::appendAttrPostOps(const ActivationPostOp& postOp, bool isLastPostOp, bool allowBinary) {
...
bool DnnlPostOpsComposerNew::appendAttrPostOps(const ScaleShiftPostOp& postOp, bool isLastPostOp, bool allowBinary) {
    ...
    case ScaleShiftPostOp::Type::prelu:
        ...
        break;
    default:
        IE_THROW() << postOp.type << " as post operation is not supported";
    }
...
static OptimizedFormula updateOptimizedFormula(const FakeQuantizePostOp& postOp, bool do_rounding) {
    ...
    auto isPerTensor = [](const std::vector<float>& v, float ref, const float zero_thr = std::numeric_limits<float>::min()) {
        return std::all_of(v.cbegin(), v.cend(), [&](float val) {
            return abs(val - ref) < zero_thr;
        });
    };
    size_t OC = std::max({inputScale.size(), inputShift.size(), cropLow.size(), cropHigh.size(), outputScale.size(), outputShift.size()});
...
bool DnnlPostOpsComposerNew::appendAttrPostOps(const FakeQuantizePostOp& postOp, bool isLastPostOp, bool doRounding, bool allowBinary) {
    DEBUG_LOG("isLastPostOp=", isLastPostOp, ", outDataType=", outDataType, ", allowBinary=", allowBinary, ", doRounding=", doRounding);
    ...
    if (isLastPostOp && (postOp.levels == 256) && f.clo.size() == 1 && f.chi.size() == 1 && f.osc.empty() && f.osh.empty()) {
    ...
    // return false before committing any change to DnnlPostOpsComposerNew
    if (!allowBinary) {
        if (f.ish.size() > 1)
            return false;
...
void DnnlPostOpsComposerNew::compose() {
    for (size_t i = 0; i < postOps.size(); ++i) {
        ...
        IE_THROW() << "Fusing of operation to postOp is not implemented";
    }

    attr.set_post_ops(ops);
}

void DnnlPostOpsComposerNew::updateWeiScales() {
...
void DnnlPostOpsComposerNew::updateDestScales() {
...
void DnnlPostOpsComposerNew::appendBinary(const dnnl::algorithm alg, const std::vector<float>& data) {
...
void DnnlPostOpsComposerNew::appendEltwise(const dnnl::algorithm alg, float alpha, float beta) {
    DEBUG_LOG("Append eltwise post op with algorithm: ", convert_to_c(alg));
    ops.append_eltwise(alg, alpha, beta);
}

void DnnlPostOpsComposerNew::appendRoundHTE() {
    appendEltwise(dnnl::algorithm::eltwise_round_half_to_even, 0, 0);
}

bool DnnlPostOpsComposerNew::appendScale(const std::vector<float>& scale, bool isLastPostOp, bool allowBinary) {
...
bool DnnlPostOpsComposerNew::appendShift(const std::vector<float>& shift, bool allowBinary) {
    if (shift.size() == 1) {
        if (shift[0] != 0.0f) {
...
bool DnnlPostOpsComposerNew::appendLinear(const std::vector<float>& scale, const std::vector<float>& shift, bool isLastPostOp, bool allowBinary) {
...
void DnnlPostOpsComposerNew::appendClip(const std::vector<float>& low, const std::vector<float>& high) {
