...
public:
    ...
    bool resize(size_t size, const Shape& shape) override {
        return m_pBlock->resize(size, shape);
    }
    bool hasExtBuffer() const noexcept override {
        return m_pBlock->hasExtBuffer();
    }
...
private:
    ...
    void allocate() override {
        if (m_workspace) m_workspace->resize(m_totalSize, Shape{});
    }
    void release() override {
...
edgeClusters MemoryControl::findEdgeClusters(const std::vector<EdgePtr>& graphEdges) {
    ...
    auto getNuma = [](const EdgePtr& edge) {
        return edge->getParent()->getNumaId();
    };
    ...
    for (const auto& edge_cluster : edge_clusters) {
        ...
        const auto single_numa_cluster = std::all_of(edge_cluster.begin(), edge_cluster.end(),
                                                     [&any_edge](const EdgePtr& edge) {
                                                         return edge->getChild()->getNumaId() == any_edge->getChild()->getNumaId();
                                                     });
        if (!single_numa_cluster) {
            std::cout << "Not all the edges in a cluster are the same!!!" << "\n";
