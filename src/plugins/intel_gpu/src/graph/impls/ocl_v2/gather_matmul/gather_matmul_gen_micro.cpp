...
gathermatmul_config GatherMatmulMicroGenerator::get_config(const kernel_impl_params& params) {
    ...
    if (std::any_of(quantized_types.begin(), quantized_types.end(), [&](const cldnn::data_types& t) -> bool {
        ...
        if (static_cast<int32_t>(params.input_layouts.size()) > cfg.weight_zp_idx &&
            params.input_layouts[cfg.weight_zp_idx].count() > 0) {
...
JitConstants GatherMatmulMicroGenerator::get_jit_constants(const kernel_impl_params& params, const micro::Package& bgm_gemm, const gathermatmul_config& cfg) const {
    ...
    // OUTPUT_STRIDE = N dimension of weights (number of output features)
    jit.make("OUTPUT_STRIDE", weight_shape[1]);
